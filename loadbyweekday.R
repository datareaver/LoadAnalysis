loadbyweekday <- function(x,name = '',origin = '02/01/2013 00:00',warmup = 14) {
    require(chron)
    require(reshape2)
    ...
    sumtable <- data.frame(ddply(finaltimetable,.(Weekday),summarize,Load = sum(LoadxTime)/sum(StateTime)))
    ...
    tiff(paste('PatientLoadbyWD',ifelse(name != '','.',''),name,'.tiff',sep = ''),800,600)
    par(mfrow = c(1,1))
    plot(sumtable[,2],col = 2:8,xlab = 'Day of Week',ylab = 'Mean Patient Load',
         ylim = c(0,max(sumtable[,2])),type = 'h',axes = F,lwd = 50,lend = 1)    
    axis(1,at = 1:7,labels = c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"))
    axis(2)
      dev.off()
    write.csv(sumtable,paste('LoadTablebyWD',ifelse(name != '','.',''),name,'.csv',sep = ''))
}
