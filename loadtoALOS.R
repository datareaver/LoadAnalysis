calc.mean.alos <- function(ts,warmup = 14) {
  #ts is two columns of timestamps--Arrivals & Departures
  require(dplyr)
  require(lubridate)
  ...
  dt <- rbind(arrivals,departures) %>% 
    arrange(Timestamp) %>%
    mutate(Hours.In.State = as.numeric(lead(Timestamp)-Timestamp)/60/60) %>%
  ...
  results <-  data.frame(dt,Patients.In.System = cumsum) %>%
    #filter(Timestamp > min(ts[,1]) + (warmup*60*60*24)) %>%
    mutate(Departure = Change < 0,Denominator = 1) %>% 
    group_by(Patients.In.System) %>% 
    summarize(ArrivalRate = (sum(Denominator)-sum(Departure))/(mean(Hours.In.State)*sum(Denominator)),
              DepartureRate = sum(Departure)/(mean(Hours.In.State)*sum(Denominator)),
              N = sum(Denominator)) %>%
    mutate(MeanLOS = (Patients.In.System/DepartureRate))
